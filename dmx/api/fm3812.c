...
//	if ( freq & 0x00FF != VoiceFreq[ Voice ] & 0x00FF )
...
*/
	if ( ( inst->fFNoteDVibVoc2 & FLG_VOICE2 ) != 0 )
	{
		...
		{
/*
			if ( Channel >= ( WORD ) PolyCutOff && Channel != 15 )
				return;
*/
			while ( j < 2 )
			{
				...
				{
					p = &Poly[ PolyActive[ j ] ];
					if ( p->Channel == Channel || p->Patch == Patch )
					{
						FmPolyNoteOff( j );
						break;
					}
