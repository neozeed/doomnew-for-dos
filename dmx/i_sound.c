...
void I_sndArbitrateCards(void)
{
  ...
  boolean codec, ens, gus, adlib, pc, sb, midi;
  ...
  if ( ens )
  {
     if (devparm) printf("ENSONIQ\n");
     if (ENS_Detect()) printf("Dude.  The ENSONIQ ain't responding.\n" );
  }
  if (codec)
  {
    if (devparm) printf("CODEC p=0x%x, d=%d\n",
      snd_SBport, snd_SBdma );

    if (CODEC_Detect(&snd_SBport,&snd_SBdma)) printf("CODEC.  The CODEC ain't responding.\n");
  }
  if (gus)
  {
